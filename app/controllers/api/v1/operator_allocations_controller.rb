...
last_ops = OperatorAllocation.where(:shifttransaction_id=>@operator_allocation.shifttransaction_id, :machine_id=>@operator_allocation.machine_id,:tenant_id=>@operator_allocation.tenant_id).last
 if last_ops != nil
  ...
  def create
	@operator_allocation = OperatorAllocation.new(operator_allocation_params)
    	@operator_allocation.operator_allot_detail.start_time = @operator_allocation.shifttransaction.shift_start_time
	@operator_allocation.operator_allot_detail.end_time = @operator_allocation.shifttransaction.shift_end_time	 
	if @operator_allocation.save
		render json: {response: @operator_allocation, status: created, msg: "created successfully"}, status: :true
	else
...
#    @operator_allocation.destroy
  ...
  private
    ...
    def operator_allocation_params
      params.require(:operator_allocation).permit(:operator_id, :shifttransaction_id, :machine_id, :description,:tenant_id,:date, operator_allot_detail: [:operation_allocation_id, :operation_management_id, :operator_id, :start_time, :end_time])
    end
end
end
end
