...
class UsersController < ApplicationController
  before_action :set_user, only: [:show, :update, :destroy, :welcome]
  skip_before_action :authenticate_request, only: %i[email_validation check_status]
...
#    users = User.where(tenant_id: @current_user.tenant_id).where.not(id: @current_user.tenant_id)
  ...
  def welcome
	if @user.present?
 		PasswordMailer.change_password(@user.user_id).deliver_now
		render json: true
	else
		render json: false
	end		
